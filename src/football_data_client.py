...
Copre: Premier League, Serie A, La Liga, Bundesliga, Ligue 1, Eredivisie,
Primeira Liga, Champions League, Europa League, Conference League,
Brasileirão, Allsvenskan, Eliteserien, Veikkausliiga, Championship, ecc.
"""
import os
import json
import requests
...
# Mappa competizioni → ID Football-Data
COMPETITIONS = {
...
class FootballDataClient:
    ...
    def _get(self, endpoint, params=None):
        ...
        try:
            ...
            if r.status_code == 429:
                print(f"[!] Rate limit su {endpoint}, salto...")
                return None
            r.raise_for_status()
            return r.json()
    ...
    def get_recent_matches(self, comp_id, days_back=60):
        """Ultimi match per calcolare forma e Elo."""
    ...
    def get_team_matches(self, team_id, limit=10):
        """Ultimi match di una squadra per la forma."""
        ...
        if not data:
            return []
        results = []
        for m in data.get("matches", [])[:5]:  # ultime 5
...
def fetch_all_data():
    """Scarica tutto per tutte le competizioni."""
    ...
    all_matches = []  # per Elo globale

    for comp_id, meta in COMPETITIONS.items():
        print(f"[→] Scarico {meta['name']} ({comp_id})...")
        ...
        if not teams:
            print(f"    [!] Nessun dato squadre per {comp_id}, salto...")
            continue

        # Forma per ogni squadra
...
if __name__ == "__main__":
    ...
    print(f"\n✅ Salvato raw_football_data.json ({len(data['competitions'])} competizioni)")
